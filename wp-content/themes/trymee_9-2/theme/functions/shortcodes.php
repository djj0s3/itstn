...
function cmsmasters_tabs($atts, $content = null) {
    ...
    foreach ($atts as $tab) {
        $out .= '<li>' . 
            '<a href="#" class="button"><span>' . $tab . '</span></a>' . 
        '</li>';
    }
...
function cmsmasters_accordion($atts, $content = null) {
    ...
    $content = str_replace('<a class="tog" href="#">', '<a class="tog button_medium" href="#">', do_shortcode($content));
			...
			if ($show_images == 'true') {
				if ($type == 'slider' || $type == 'album' || $type == 'gallery') {
					if (has_post_thumbnail()) {
						...
						foreach ($attachments as $attachment) {
							if (!isset($counter) && $counter = true) {
								$out .= cmsms_thumb(get_the_ID(), 'project-thumb', true, false, true, false, true, false, true, $attachment->ID);
							}
						}
					} else {
						$out .= '<figure>' . 
							'<a class="preloader" href="' . get_permalink() . '"' . ' title="' . cmsms_title(get_the_ID(), false) . '">' . 
								'<img src="' . get_template_directory_uri() . '/images/PT-gallery.jpg' . '" alt="' . cmsms_title($cmsms_id, false) . '" title="' . cmsms_title($cmsms_id, false) . '" class="fullwidth" />' . 
							'</a>' . 
						'</figure>';
					}
				} else {
					if (has_post_thumbnail() && $type != 'video') {
						...
						$out .= '<figure>' . 
							'<a class="preloader" href="' . get_permalink() . '"' . ' title="' . cmsms_title(get_the_ID(), false) . '">' . 
								'<img src="' . get_template_directory_uri() . '/images/PT-' . (($type == 'image' || $type == '') ? 'placeholder' : $type) . '.jpg' . '" alt="' . cmsms_title($cmsms_id, false) . '" title="' . cmsms_title($cmsms_id, false) . '" class="fullwidth" />' . 
							'</a>' . 
						'</figure>';
					}
				}
