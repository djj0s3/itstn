...
function register_js_scripts() {
    if (!is_admin()) {
        ...
        wp_register_script('jquery', get_template_directory_uri() . '/js/jquery.min.js', array(), '1.7.2', false);
...
function the_widgets_init() {
    ...
    register_sidebar(
        array(
            'name' => __('Colored Sidebar', 'cmsmasters'), 
            'id' => 'sidebar_colored', 
            'description' => __('Widgets in this area will be shown in colored page template at the top of middle block, above the content.', 'cmsmasters'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
